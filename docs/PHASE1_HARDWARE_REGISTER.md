...
| Locating-hole geometry | TBD / BLOCKED | Exact diameter, plating, copper, and coordinates are not frozen. | Header and final module geometry have not been physically verified | High confidence that evidence is missing | Fit-test actual temporary header against a true-scale footprint and fabricator tolerances. | PCB layout + Fabrication |
...
| Item | Status | Current value / decision | Evidence / source | Confidence | Remaining validation | Blocks |
|---|---|---|---|---|---|---|
| Production power path | DESIGN DECISION | Protected 24 V rail → onboard 24 V-to-5 V regulator → regulated 5 V → ESP32-S3 Super Mini external 5 V/VBUS input. Raw 24 V must never reach the module. | Phase-1 power architecture decision | High | Verify the module power pin and internal USB/external-5 V topology; select and qualify the regulator and source isolation. | PCB layout + BOM freeze + Fabrication |
| USB-C role | DESIGN DECISION | Programming, debug, recovery, and service only; not the normal installed power source. | Phase-1 service architecture decision | High | Validate every USB-only and mixed-supply power state. | PCB layout + Fabrication |
...
| Backfeed protection | TBD / BLOCKED | Final need and implementation for Schottky OR-ing, ideal-diode/load-switch isolation, USB-VBUS isolation, or a service-power selector is unresolved. | Depends on actual module topology and measured source behavior | High confidence that decision is blocked | Prove that USB cannot backfeed the buck/24 V system, onboard 5 V cannot unexpectedly source a USB host, and supplies cannot contend. | PCB layout + BOM freeze + Fabrication |
...
| Gate-drive concept | ASSUMPTION / PROVISIONAL | 5 V 74AHCT125, provisional 47 Ω gate resistor, and hardware gate pull-down with default-OFF intent. | Current Rev A.4.1 schematic concept | Medium | Verify power sequencing/default-OFF behavior, buffer enable strategy, partial-power paths, gate waveform, switching loss, and suitability at the chosen PWM frequency. | PCB layout + BOM freeze + Firmware constants + Fabrication |
