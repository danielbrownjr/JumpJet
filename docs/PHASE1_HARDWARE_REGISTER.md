...
| Castellation receiving lands | ASSUMPTION / PROVISIONAL | Use 18 SMD receiving lands at provisional 2.54 mm pitch and 15.24 mm row spacing. Derive land width from the measured 1.96 mm castellation width with appropriate solder/process margin; exact width, inward depth, mask expansion, paste geometry, and longitudinal registration remain unfrozen. | Physical module measurements and Phase-1 direct-solder architecture | Medium | Complete footprint/process review and pass a 1:1 physical fit check. | PCB layout + Fabrication |
...
| Central underside PCB window | ASSUMPTION / PROVISIONAL | **Current decision state: REEVALUATION REQUIRED.** Underside component clearance no longer justifies a window. A solid carrier region with controlled isolation and soldermask beneath the module is now the leading architecture to evaluate, with no exposed carrier copper or vias where underside contacts could touch. Retain a window only if cleaning, inspection, thermal, assembly, or mechanical review supplies an independent justification. | Direct inspection confirms a flat underside with no mounted components, but with exposed electrical/test contacts | Medium | Compare solid-carrier and independently justified window options; map underside contacts; review isolation, cleaning, inspection, thermal, assembly, and structural needs. | Footprint freeze + PCB layout + Fabrication |
| Underside window geometry | TBD / BLOCKED | No window geometry is frozen because the need for a window has been reopened. If retained for an independently validated reason, its length, width, corners, and support rails must be defined. | Central-window reevaluation | High confidence that no geometry is approved | Complete the architecture review before defining any opening. | PCB layout + Fabrication |
...
| Locating-hole geometry | ASSUMPTION / PROVISIONAL | Candidate holes are Ø1.10 mm NPTH. Previously proposed X = −7.620 mm, Y = ±12.700 mm coordinates are not usable or frozen until the footprint origin and orientation datum are explicitly documented and physically checked. | Phase-1 assembly-aid review using a provisional nominal square-header concept | Low-medium | Document the datum/orientation; measure the actual header; verify finished-hole and positional tolerances; physically fit-test insertion/removal; check interference with receiving lands, antenna, and USB. | PCB layout + Fabrication |
...
| Antenna keepout | ASSUMPTION / PROVISIONAL | Prefer the antenna at a carrier-board edge. Reserve an RF keepout around and beneath it from copper, planes, vias, traces, heater-current copper, switch-node copper, fuse hardware, and large grounded metal; preserve enclosure clearance where practical. Exact geometry and affected layers remain TBD. | Likely antenna location plus Phase-1 RF layout direction | Medium | Define the exact keepout and validate Wi-Fi RSSI/range after final PCB placement and again after enclosure installation. | PCB layout + Fabrication |
...
- ESP32 carrier receiving-land geometry and exact body/pad registration
...
- central carrier-region/window reevaluation; window/support-rail geometry only if a window remains independently justified
- antenna identity, exact keepout, and installed RSSI/range validation
